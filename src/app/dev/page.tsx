    ...
    <p style={{ textAlign: "center", color: "gray", fontStyle: "italic" }}>
      ...
      <section className="space-y-6 text-left mb-12">
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-8 border border-green-200">
          <h2 className="text-4xl font-bold text-foreground mb-4">개발 💻</h2>
          <p className="text-muted text-lg leading-relaxed">
            프로그래밍과 개발 관련 글들을 모았습니다
          </p>
          <div className="flex flex-wrap gap-3 mt-6">
            {["JavaScript", "Python", "Java", "React", "Node.js"].map((tech) => (
              <span
                key={tech}
...
<p style={{ textAlign: "center", color: "gray", fontStyle: "italic" }}>squirrel in Seattle</p> 
      ...
      <section className="space-y-8">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-foreground">개발 글</h2>
        ...
        <div className="grid gap-6">
          {filteredPosts.map(({ id, date, title, excerpt, tags }) => (
            <Link key={id} href={`/posts/${id}`} className="block group">
              <article className="relative bg-card-background border border-card-border rounded-xl p-6 hover:shadow-lg hover:border-green-300 transition-all duration-300 cursor-pointer group-hover:-translate-y-1">
                <div className="space-y-4">
                  ...
                  <h3 className="text-xl font-bold text-foreground group-hover:text-green-600 transition-colors leading-tight">
                    {title}
                  </h3>
                  ...
                  {tags && tags.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {tags.map(tag => (
                        <span key={tag} className="text-xs px-3 py-1 bg-green-100 text-green-700 rounded-full border border-green-200">
                          {tag}
                        </span>
                      ))}
                    </div>
